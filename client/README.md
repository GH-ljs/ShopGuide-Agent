...
- 按 `meta type=fallback` 展示模型降级轻提示。
...
- 连接后端健康检查接口，顶栏显示连接状态。
- 聊天窗口支持中文输入、500 字输入上限、发送中禁用、错误提示和快捷问题。
- 使用 SSE 渲染流式回答。
...
- 支持发送失败重试：网络、后端或检索异常时，在用户消息气泡前显示红色重试图标。
- 支持模型降级提示：LLM 不可用但后端已用本地规则完成推荐时，显示轻提示而不标记发送失败。
- 支持图片加载失败占位：商品图无法加载时显示“暂无图片”占位，不让卡片出现空白。
...
输入框在 `InputBar.kt` 中会把用户本轮输入限制到 500 字，并显示字数计数。这个限制主要是为了避免误粘贴大段文本拖慢 UI、请求体、Prompt 和检索；后端仍会再次校验同样的长度，防止旧客户端或脚本绕过端侧限制。
...
| `meta` | 缓存、首 token 和降级提示 | `fallback` 时展示模型降级轻提示，其他调试信息可忽略 |
...
客户端把异常分成两类：**发送失败**和**模型降级**。

### 发送失败

发送失败表示本轮客户端没有拿到完整可用的导购结果。常见情况包括：

| 场景 | 客户端错误码 | 展示效果 |
| --- | --- | --- |
| 后端没启动、地址错误、模拟器/真机访问不到 `BASE_URL` | `NETWORK_ERROR` | 用户消息气泡前显示红色重试图标，助手消息显示“无法连接后端服务” |
| 连接超时、读取超时、SSE 中途断开 | `NETWORK_ERROR` | 用户消息气泡前显示红色重试图标 |
| 后端 HTTP 非 2xx | `HTTP_ERROR` | 用户消息气泡前显示红色重试图标 |
| 后端 SSE `error`：检索异常 | `RETRIEVAL_ERROR` | 用户消息气泡前显示红色重试图标 |
| 后端 SSE `error`：参数或内部错误 | `VALIDATION_ERROR` / `INTERNAL_ERROR` / `INVALID_JSON` | 用户消息气泡前显示红色重试图标 |

失败图标在 `MessageBubble.kt` 中绘制为红色环形箭头。点击后会用原用户消息重新请求 `/api/chat`，同时移除上一轮紧跟着的错误提示，避免界面同时保留失败提示和新回答。

重试时，客户端不会把失败的用户消息和错误助手消息再次放入 `history`。这是为了避免后端把“错误码”“连接失败”等客户端状态误当成真实购物需求。

### 模型降级

LLM 生成失败不再默认算发送失败。后端会在商品检索成功的前提下退回本地导购规则回答，并发送：
...
客户端收到后，会在当前 AI 回复下方展示轻提示：

```text
当前 AI 生成服务暂时不可用，已使用本地导购规则完成推荐。
```

这种情况下仍会正常显示回答文本和商品卡片，不显示红色重试图标。
...
商品卡片和详情页图片都通过 `RemoteImage.kt` 加载。图片加载过程分三种状态：

| 状态 | 展示效果 |
| --- | --- |
| 加载中 | 固定尺寸浅灰占位，避免列表跳动 |
| 加载成功 | 展示真实商品图片 |
| 加载失败 | 展示浅灰背景、图片图标和“暂无图片” |

加载失败通常来自图片接口 404、网络断开、图片地址拼接错误或图片格式解码失败。占位图只表示“图片暂不可用”，商品标题、价格、类目和详情点击仍然以结构化商品数据为准。
