...
- `src/memory.js`: 内存多轮会话记忆

## Data Flow

1. 服务启动时读取 `ecommerce_agent_dataset` 下所有商品 JSON。
2. 用户请求 `/api/chat`。
3. 后端根据 `conversationId` 读取最近会话历史和上一轮商品。
4. 后端根据类目、商品类型、预算、否定词做候选过滤。
5. 本地向量检索器把用户问题、历史需求和商品文本转成词频向量，用余弦相似度排序。
6. 如果配置了 `ARK_API_KEY`，把候选商品和最近对话作为上下文发给 Doubao。
7. 如果没有模型 Key，使用本地规则生成兜底回复。
8. SSE 先返回 `token` 流式文本，再返回 `products` 商品卡片。
9. 回答结束后，把用户消息、助手回复和本轮商品写入内存会话。
